...
import { RotateCw, Search, ShieldCheck, Award, ChevronRight, UserCheck } from 'lucide-react';
  ...
  return (
    <div className="revolut-card bg-[#16171B] border border-white/[0.08] rounded-[26px] p-5 sm:p-6 flex flex-col h-[480px] space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-base font-bold text-white tracking-tight">Active Index Whales</h3>
          <p className="text-xs text-[#8E8F99]">Mirrored Polymarket accounts</p>
        </div>
        <button
          onClick={handleReevaluate}
          disabled={evaluating}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-[#1C1D22] hover:bg-[#2C2D35] text-white text-xs font-semibold rounded-full border border-white/10 transition-all cursor-pointer disabled:opacity-50"
        >
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex rounded-full bg-[#1C1D22] p-0.5 border border-white/5 text-[11px] font-bold">
          <button
            ...
            className={`px-3 py-1 rounded-full transition-all ${tab === 'copied' ? 'bg-[#2C2D35] text-white' : 'text-[#8E8F99]'}`}
          >
            Copied
          </button>
          <button
            onClick={() => setTab('gold')}
            className={`px-3 py-1 rounded-full transition-all ${tab === 'gold' ? 'bg-[#2C2D35] text-white' : 'text-[#8E8F99]'}`}
          >
            Gold Snipers
          </button>
          <button
            onClick={() => setTab('all')}
            className={`px-3 py-1 rounded-full transition-all ${tab === 'all' ? 'bg-[#2C2D35] text-white' : 'text-[#8E8F99]'}`}
          >
            All
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pr-1 divide-y divide-white/5">
        {loading ? (
          <div className="flex flex-col items-center justify-center h-48 text-[#8E8F99] text-xs">
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-full bg-[#2C2D35] border border-white/10 flex items-center justify-center font-bold text-sm text-white shrink-0 uppercase relative">
                    {name.slice(0, 2)}
                    {isGold && (
                      <span className="absolute -top-0.5 -right-0.5 w-3 h-3 rounded-full bg-amber-400 border-2 border-[#16171B]" />
                    )}
                  </div>
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {isGold && (
                        <span className="text-[9px] bg-amber-400/10 text-amber-400 border border-amber-400/20 px-1.5 py-0.2 rounded-full font-bold">
                          Gold
                        </span>
                      )}
                    </div>
                    <span className="text-[11px] text-[#8E8F99] font-mono block truncate">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <div className="text-right">
                    <div className={`text-xs font-bold font-mono ${pnl >= 0 ? 'text-[#00D09C]' : 'text-[#FF453A]'}`}>
                      {pnl >= 0 ? '+' : ''}${Math.abs(pnl).toLocaleString(undefined, { maximumFractionDigits: 0 })}
                    </div>
                    <span className="text-[10px] text-[#8E8F99]">Mirrored</span>
                  </div>
                  <ChevronRight size={14} className="text-[#8E8F99] group-hover:text-white transition-colors" />
                </div>
              </div>
            );
          })
