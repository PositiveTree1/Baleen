...
import { Settings, LogOut, Volume2, VolumeX, Bell, GripVertical, Maximize2, Minimize2, SlidersHorizontal, RotateCcw, ArrowUp, ArrowDown, Check } from 'lucide-react';
import { signOut } from 'next-auth/react';
import { BrandLogo } from '@/components/ui/BrandLogo';
import { soundFx } from '@/lib/sound';

const DEFAULT_WIDGETS = ['analytics', 'tape_whales', 'positions'];

const WIDGET_TITLES: Record<string, string> = {
  analytics: 'Portfolio Capital & Performance',
  tape_whales: 'Live Orders & Active Whale Index',
  positions: 'Execution Audit & Open Positions',
};
...
export default function DashboardPage() {
  ...
  // Revolut-Style Customizable Widgets State
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('baleen_widget_order_v1');
      if (saved) {
      ...
      const savedExp = localStorage.getItem('baleen_widget_expanded_v1');
      if (savedExp) {
        setExpandedWidgets(JSON.parse(savedExp));
      }
    } catch {}
  }, []);
  ...
  const handleReorder = (newOrder: string[]) => {
    ...
    try {
      localStorage.setItem('baleen_widget_order_v1', JSON.stringify(newOrder));
    } catch {}
  };
  ...
  const toggleExpand = (id: string) => {
    setExpandedWidgets(prev => {
      ...
      try {
        localStorage.setItem('baleen_widget_expanded_v1', JSON.stringify(next));
      } catch {}
      return next;
    });
  };
  ...
  const resetWidgets = () => {
    handleReorder(DEFAULT_WIDGETS);
    setExpandedWidgets({});
    try {
      localStorage.removeItem('baleen_widget_expanded_v1');
    } catch {}
  };
  ...
  const renderWidgetContent = (id: string) => {
    switch (id) {
      ...
      case 'tape_whales':
        return (
          <div className={expandedWidgets['tape_whales'] ? "grid grid-cols-1 gap-6" : "grid grid-cols-1 lg:grid-cols-3 gap-6"}>
            <div className={expandedWidgets['tape_whales'] ? "w-full" : "lg:col-span-2"}>
              <LiveTape userId={session?.user?.id} onSelectTrade={setSelectedTrade} />
            </div>
            <div className={expandedWidgets['tape_whales'] ? "w-full" : "lg:col-span-1"}>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F8F9FB] text-slate-900 selection:bg-slate-900 selection:text-white relative overflow-x-hidden">
      {/* Clean Modern Liquid Glass Specular Ambience */}
      ...
      <main className="flex-1 p-6 lg:p-12 max-w-7xl mx-auto w-full flex flex-col gap-6 relative z-10">
        {/* Header Section with Balance & Revolut Widget Customizer */}
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 pb-2">
          ...
          <div className="flex items-center gap-2.5 flex-wrap">
            {/* Revolut Customizer Toggle */}
            <button
              onClick={() => setIsCustomizing(!isCustomizing)}
              className={`flex items-center gap-1.5 px-3 py-2 rounded-2xl text-xs font-semibold border transition-all cursor-pointer shadow-xs ${
                isCustomizing
            ...
            >
              ...
              <span>{isCustomizing ? 'Done Customizing' : 'Customize Layout'}</span>
            </button>

            {isCustomizing && (
              <button
                onClick={resetWidgets}
                className="flex items-center gap-1 px-3 py-2 rounded-2xl text-xs font-semibold bg-white/80 hover:bg-white text-slate-600 hover:text-slate-900 border border-black/[0.08] transition-colors cursor-pointer shadow-xs"
        ...
        {/* Customizable Reorderable Widget Area */}
        <Reorder.Group
          axis="y"
          values={widgetOrder}
            ...
            return (
              <Reorder.Item
                ...
                className={`relative rounded-3xl transition-shadow ${
                  isCustomizing ? 'border-2 border-dashed border-indigo-400/80 bg-indigo-50/20 p-2 shadow-lg' : ''
                }`}
              >
                {/* Widget Action Bar (Visible when customizing or on hover) */}
                {isCustomizing && (
                  <div className="flex items-center justify-between px-3 py-2 mb-2 bg-white/90 backdrop-blur-md rounded-2xl border border-black/[0.08] shadow-sm">
                    <div className="flex items-center gap-2 text-slate-700">
                      <GripVertical size={16} className="text-slate-400 cursor-grab active:cursor-grabbing" />
                      <span className="text-xs font-bold font-mono tracking-tight">{WIDGET_TITLES[id] || id}</span>
                    </div>
