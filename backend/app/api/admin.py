...
from app.database import get_db, is_production_environment
from app.config import settings
from app.models import Wallet, User, ExecutionLog
from app.auth import require_admin, verify_listener_service_key
...
async def trigger_discovery(_admin: User = Depends(require_admin)):
    """Triggers discovery worker in background."""
...
async def get_admin_status(
    ...
    # Listener status
    listener_online = (time.time() - last_listener_heartbeat) < 60 if last_listener_heartbeat > 0 else False
    listener_status = "ONLINE" if listener_online else ("UNKNOWN" if last_listener_heartbeat == 0.0 else "OFFLINE")
...
async def get_discovery_progress(_admin: User = Depends(require_admin)):
...
async def purge_and_rescan(
    _admin: User = Depends(require_admin),
    db: AsyncSession = Depends(get_db)
):
    """
    Hard-wipes all existing wallets and starts background discovery from scratch.
    Disabled in production unless explicit override flag is configured.
    """
    if is_production_environment() and not getattr(settings, "ALLOW_DESTRUCTIVE_ADMIN_WIPE", False):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Purge and rescan is disabled in production environment."
        )
    ...
    await hard_wipe_all_database(_admin=_admin, db=db)
...
async def hard_wipe_all_database(
    _admin: User = Depends(require_admin),
    db: AsyncSession = Depends(get_db)
):
    """
    ...
    if is_production_environment() and not getattr(settings, "ALLOW_DESTRUCTIVE_ADMIN_WIPE", False):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Hard wipe is disabled in production environment."
        )

    from app.models import ExecutionLog, PortfolioSnapshot, WalletSnapshot, Wallet, FeeCharge, KeyValue, User, ExposureLedger
...
async def export_trades_csv(
    ...
    # Audit exports contain stored evidence, including archived runs. Current
    # market estimates cannot rewrite the financial history of a prior run.
    stmt = select(ExecutionLog).execution_options(include_archived_runs=True).order_by(ExecutionLog.executed_at.desc())
    logs = (await db.execute(stmt)).scalars().all()
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(['Execution ID', 'User ID', 'Run ID', 'Evidence', 'Timestamp UTC',
        'Source Whale Address', 'Market Title', 'Condition ID', 'Side', 'Outcome',
        'Recorded Fill Price USD', 'Recorded Notional USD', 'Recorded Fee USD',
        'Recorded Realized PnL USD', 'Recorded Status', 'Tx Hash'])
    for row in logs:
        writer.writerow([str(row.id), str(row.user_id) if row.user_id else '',
            str(row.run_id) if row.run_id else '', 'paper_simulation' if row.is_sandbox else 'legacy_unverified',
            row.executed_at.isoformat() if row.executed_at else '', row.source_wallet_address,
            row.market_question, row.market_condition_id, row.side, row.resolution_outcome,
            row.user_fill_price, row.notional_usd, row.fee_usd, row.realized_pnl_usd,
            row.status, row.onchain_tx_hash])
